...
try {
    ...
    foreach ($fragment in @(
            'setup-ui\runtime\Project Setup.exe',
            'start "" /wait',
    ...
    if ($launcherText.IndexOf('setup-ui\runtime\Project Setup.exe', [System.StringComparison]::Ordinal) -gt
        $launcherText.IndexOf('where pwsh.exe', [System.StringComparison]::Ordinal)) {
        throw 'Windows-запускатель проверяет системный PowerShell раньше автономного мастера.'
    }
    ...
    if ([regex]::Matches($wizardHtml, 'data-step-panel=').Count -ne 6 -or
        ...
        $wizardHtml -notmatch 'id="maintenance"' -or
        $wizardHtml -notmatch 'id="disable-sync-button"' -or
        $wizardHtml -notmatch 'id="open-sync-log-button"' -or
        $wizardHtml -notmatch 'id="inspect-tools-button"' -or
        $wizardHtml -notmatch 'id="guidance-heading"' -or
        $wizardHtml -notmatch 'id="technical-error"' -or
    ...
    foreach ($pattern in @('setup:inspect-tools', 'setup:open-guide', 'setup:open-project-guide', 'setup:open-obsidian', 'setup:set-local-sync', 'setup:open-local-sync-log')) {
    ...
    foreach ($technicalCommand in @('irm https://', 'npm install -g', 'curl -fsSL')) {
        if ($toolsScriptText -match [regex]::Escape($technicalCommand)) {
            throw "Мастер показывает руководителю техническую команду установки: $technicalCommand"
        }
    }
    foreach ($pattern in @('inspectTools:', 'openGuide:', 'openProjectGuide:', 'openObsidian:', 'setLocalSync:', 'openLocalSyncLog:')) {
    ...
    foreach ($pattern in @(
            'currentStep === 4',
            'showStep(5)',
            'openProjectGuide(button.dataset.projectGuide)',
            'friendlyFailure',
            'defaults.runtimeLabel',
            'renderLocalSyncState',
            'setLocalSyncState'
        )) {
    ...
    $runtimeBuilderText = [System.IO.File]::ReadAllText((Join-Path $root 'scripts/build-setup-runtime.ps1'))
    foreach ($pattern in @('$PSHOME', "'powershell'", "'pwsh.exe'", 'Встроенный PowerShell')) {
        if ($runtimeBuilderText -notmatch [regex]::Escape($pattern)) {
            throw "Сборщик автономного мастера не содержит обязательный фрагмент: $pattern"
        }
    }
...
'@
    ...
    $startProjectText = [System.IO.File]::ReadAllText((Join-Path $source 'scripts/start-project.ps1'))
    if ([regex]::Matches($startProjectText, 'Invoke-ConsoleWizard').Count -ne 2) {
        throw 'Текстовый мастер должен запускаться только по явному параметру -Console.'
    }
    ...
    $reportInvalid = @(
        ...
        [string]$report.localSync.status -cne 'disabled-policy'
