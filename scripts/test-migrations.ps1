...
function New-ProjectFixture(
    ...
    foreach ($relative in @(
        ...
        if ($Version -cne '0.8.0') { Remove-SafePath $project $relative }
    }
...
try {
    ...
    if ($state070.previousTemplateVersion -cne '0.7.0' -or $state070.templateVersion -cne '0.8.1') {
        throw 'TEMPLATE-STATE.json не зафиксировал переход 0.7.0 -> 0.8.1.'
    }

    $project060 = New-ProjectFixture 'from-060' '0.6.0'
    & $updater -ProjectPath $project060 -Date '2026-07-16' -Apply
    Assert-Version $project060 '0.8.1'
    ...
    if ($state060.previousTemplateVersion -cne '0.6.0' -or $state060.templateVersion -cne '0.8.1') {
        throw 'TEMPLATE-STATE.json не зафиксировал переход 0.6.0 -> 0.8.1.'
    }
    ...
    Assert-Version $project050 '0.8.1'
    ...
    if ($state050.previousTemplateVersion -cne '0.5.0' -or $state050.templateVersion -cne '0.8.1') {
        throw 'TEMPLATE-STATE.json не зафиксировал переход 0.5.0 -> 0.8.1.'
    }
    ...
    Assert-Version $project040 '0.8.1'
    ...
    Assert-Version $project030 '0.8.1'
    ...
    Assert-Version $project020 '0.8.1'
    ...
    Write-Host 'Сценарии миграции проектов до 0.8.1 пройдены.'
}
finally {
